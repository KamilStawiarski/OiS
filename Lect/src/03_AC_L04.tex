...
\begin{frame}{Impedancja operatorowa}
	\textbf{Impedancja operatorowa} - stosunek transformaty Laplace'a napięcia na elemencie do transformaty Laplace'a prądu płynącego przez ten element, przy zadanych warunkach początkowych.\\
	Dla zerowych warunków początkowych ma postać:
	\begin{equation*}
		Z(s) = \frac{U(s)}{I(s)}
	\end{equation*}
	\begin{itemize}
		\item[\bt] Rezystor: $u_R(t) = R i_R(t) \Rightarrow U_R(s) = R I_R(s) \Rightarrow Z_R(s) = R$
		\item[\bt] Kondensator: $i_C(t) = C \frac{d u_C(t)}{dt}$ \\
		      $I_C(s) = C \left( sU_C(s) - u_C(0^{-}) \right) \Rightarrow \bigg| u_C(0^{-}) = 0 \bigg| \Rightarrow Z_C(s) = \frac{U_C(s)}{I_C(s)} = \frac{1}{sC}$
		\item[\bt] Induktor: $u_L(t) = L \frac{d i_L(t)}{dt}$ \\
		      $U_L(s) = L \left( sI_L(s) - i_L(0^{-}) \right) \Rightarrow \bigg| i_L(0^{-}) = 0 \bigg| \Rightarrow Z_L(s) = \frac{U_L(s)}{I_L(s)} = sL$
	\end{itemize}
\end{frame}
...
\begin{frame}{Od impedancji operatorowej do transmitancji}
	\begin{itemize}
		\item[\bt] Po przejściu do dziedziny $s$ elementy R, L i C można opisać za pomocą ich impedancji operatorowych: $R$, $sL$, $\frac{1}{sC}$.
		\item[\bt] Dzięki temu obwód z elementami inercyjnymi można analizować podobnie jak obwód rezystancyjny, stosując prawa Kirchhoffa i przekształcenia algebraiczne.
		\item[\bt] Po wyznaczeniu zależności między sygnałem wejściowym i wyjściowym otrzymujemy transmitancję układu.
		\item[\bt] Transmitancja opisuje właściwości dynamiczne całego układu, a nie pojedynczego elementu.
	\end{itemize}
\end{frame}
...
\begin{frame}{Transmitancja}
	...
	Zwykle oznaczana jako $G(s)$ lub $H(s)$:
	\begin{equation*}
		G(s)=\frac{Y(s)}{X(s)}
	\end{equation*}
	Transmitancja opisuje własności układu, a nie konkretnego wymuszenia.
\end{frame}
...
\begin{frame}{Transmitancja}
	...
	\begin{figure}
		\includegraphics[scale=0.07, alt={Na rysunku jest obwód elektryczny z jednym źródłem napięcia po lewej stronie oraz trzema elementami RLC, które tworzą częściowo połączenie szeregowe, a częściowo równoległe.
...
\begin{frame}{Transmitancja - układ niestabilny}
	...
	\begin{figure}
		\includegraphics[scale=0.07, alt={Na rysunku jest obwód z wzmacniaczem operacyjnym, rezystorem, kondensatorem, cewką oraz źródłem napięcia, połączonych w jednej pętli z kilkoma węzłami.
...
\begin{frame}{Uwagi}
	\begin{enumerate}
		\item[\bt] W przypadku niezerowych warunków początkowych należy zachować ciągłość wielkości magazynujących energię: $u_C(0^{-})=u_C(0^{+})$, $i_L(0^{-})=i_L(0^{+})$.
		\item[\bt] \textbf{Stan ustalony} - sytuacja, w której zakładamy, że wszystkie człony postaci $e^{-\alpha t}$ już wygasły i mają pomijalnie mały wpływ na wyjście układu.
		\item[\bt] \textbf{Transmitancja} - stosunek transformaty Laplace'a wyjścia układu do transformaty Laplace'a jego wejścia przy zerowych warunkach początkowych.
		      Zwykle oznaczana jako $G(s)$ lub $H(s)$.
	\end{enumerate}
\end{frame}